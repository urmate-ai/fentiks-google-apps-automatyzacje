...
Projekt automatyzuje przygotowywanie odpowiedzi e-mailowych dla Fentix przy użyciu Google Apps Script oraz modeli Gemini. Kontekst merytoryczny jest dostarczany przez Vertex AI Search (grounding w Gemini), dzięki czemu lokalna baza wiedzy nie jest potrzebna. Kod źródłowy znajduje się w tym katalogu.
...
1. Uzupełnij plik `GoogleScripts/01_config.js`, aby zdefiniować nazwy etykiet Gmail, konfigurację Vertex AI (np. `VERTEX_PROJECT_ID`, `VERTEX_LOCATION`, `VERTEX_MODEL`, `VERTEX_SEARCH_DATA_STORE[S]`) oraz podpisy wiadomości. W razie potrzeby można pozostawić legacy `GEMINI_API_KEY` do testów.
